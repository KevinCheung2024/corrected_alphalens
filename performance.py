...
def factor_weights(factor_data,
    ...
    weights = pd.concat(
        [to_weights(dfs,demeaned,equal_weight) for _,dfs in factor_data.groupby(grouper)['factor']]
        )

    if group_adjust:
        #weights = weights.groupby(level='date').apply(to_weights, False, False)
        weights = pd.concat(to_weights(dfs,False,False) for _,dfs in weights.groupby(level="date"))
