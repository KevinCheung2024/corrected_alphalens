...
def quantize_factor(factor_data,
    ...
    def correct_quantile_calc(x, _grouper,_quantiles,_bins,_zero_aware,_no_raise):
        #强烈建议用pd.cut()而非pd.qcut()
        def Zw1(x,qt):
            pos_qts = pd.qcut(x[x>=0],qt//2,labels=False,duplicates='drop') + qt//2 + 1
            neg_qts = pd.qcut(x[x<0],qt//2,labels=False,duplicates='drop')+1
            return pd.concat([pos_qts,neg_qts]).sort_index()
        def Zw2(x,b):
            pos_qts = pd.cut(x[x>=0],b//2,labels=False) + b//2 + 1
            neg_qts = pd.cut(x[x<0],b//2,labels=False)+1
            return pd.concat([pos_qts,neg_qts]).sort_index()
        #try:
        if _quantiles is not None and _bins is None and not _zero_aware:
            return pd.concat([pd.qcut(dfs,_quantiles,labels=False,duplicates='drop')+1 for _,dfs in x['factor'].groupby(_grouper)])
        elif _quantiles is not None and _bins is None and _zero_aware:
            return pd.concat([Zw1(dfs,_quantiles) for _,dfs in x['factor'].groupby(_grouper)])
        elif _bins is not None and _quantiles is None and not _zero_aware:
            return pd.concat([pd.cut(dfs,_bins,labels=False)+1 for _,dfs in x['factor'].groupby(_grouper)])
        elif _bins is not None and _quantiles is None and _zero_aware:
            return pd.concat([Zw2(dfs,_bins) for _,dfs in x['factor'].groupby(_grouper)])
        # except Exception as e:
        #     if _no_raise:
        #         return pd.Series(index = x.index)
        return
    ...
    #print(quantiles)
    factor_quantile = correct_quantile_calc(factor_data,grouper,quantiles, bins, zero_aware, no_raise)
...
def get_clean_factor(factor,
    ...
    #print("quantile_data",quantile_data)
    # print(merged_data)
